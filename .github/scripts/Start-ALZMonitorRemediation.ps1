...
<#
This function is used to trigger remediation on all policy sets at management group scope. 
It first calls the Azure REST API to get the policy assignments in the management group scope, then it iterates through the policy assignments, checking if it is a policy set definition, 
...
#>
Function Invoke-BatchPolicySetRemediationAtMGScope {
    Param(
    ...
    $result.value | ForEach-Object {
        #check if the policy assignment is for a policy set definition
        If ($($PSItem.properties.policyDefinitionId) -match "/providers/Microsoft.Authorization/policySetDefinitions") {
            ...
            Foreach ($policy in $policies) {
                ...
                $uri = "https://management.azure.com/providers/Microsoft.Management/managementGroups/$($managementGroupName)/providers/Microsoft.PolicyInsights/remediations/$($name)?api-version=2021-10-01"
                $body = @{
                    properties = @{
...
<#
This function is used to trigger remediation on a specific policy set at management group scope. 
It first calls the Azure REST API to get the policy assignments in the management group scope, then it iterates through the policy assignments, checking if it is a policy set definition, 
if so it extract the policy set ID, policy definition ID, and policy definition reference ID. 
Then it triggers the remediation process at management group scope using Azure REST API.
#>

Function Invoke-PolicySetRemediationAtMGScope {
    Param(
        [Parameter(Mandatory = $true)] [string] $managementGroupName,
...
Invoke-PolicySetRemediationAtMGScope -managementGroupName hubnw-platform-management -policySetName Alerting-Management
Invoke-PolicySetRemediationAtMGScope -managementGroupName hubnw-platform-identity -policySetName Alerting-Identity
Invoke-PolicySetRemediationAtMGScope -managementGroupName hubnw-platform-connectivity -policySetName Alerting-Connectivity